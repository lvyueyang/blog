---
title: canva签名插件修改于 Sketchpad  (兼容移动端)  
tag: JavaScript,ES6,Canvas  
---  

开源库地址 
[https://github.com/yiom/sketchpad](https://github.com/yiom/sketchpad)
...
class Sketchpad {
    constructor(options) {
        ...
        ['width', 'height', 'color', 'penSize', 'readOnly', 'left', 'top'].forEach(function (attr) {
        ...
        if (isMobile && this.dpi) {
            this.width = this.width * dpi
            this.height = this.height * dpi
        }
        ...
        this.internalEvents.forEach(function (name) {
            ...
            if (eventName) {
                this.canvas.addEventListener(eventName, (...args) => this.trigger(lower, args))
            }
        }, this)
        this.reset()
    }
    ...
    _position(event) {
        ...
        let position = {
            x: event.clientX - (left || this.canvas.offsetLeft),
            y: event.clientY - (top || this.canvas.offsetTop),
        }
        if (isMobile && this.dpi) {
